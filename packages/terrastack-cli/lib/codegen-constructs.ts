...
export function emitConstructForApiObject(code: CodeMaker, def: JSONSchema4, provider: string, namespace: string, version: string) {
  ...
  function emitFile() {
    ...
    if(tagType === "full") {
      code.line(`import { TagManager, TagType } from '@aws-cdk/core';`);
    }
    else if(tagType === "manager_only") {
      code.line(`import { TagManager } from '@aws-cdk/core';`);
    }
    code.line();
  ...
  function emitPropsAssignment() {
    ...
    for (const [ propName, propSpec ] of Object.entries(propType || {})) {    
      if (propSpec.referencable) {
        ...
        if (propName === 'tags') {
          code.line(`this.${propertyName(propName)} = new TagManager(TagType.MAP, '${baseName}', options.tags, { tagPropertyName: 'tags' });`);
        } else {
  ...
  function emitProperties() {
    ...
    for (const [ propName, propSpec ] of Object.entries(type || {})) {
      const propType = (propName === 'tags') ? 'TagManager' : propSpec.type
